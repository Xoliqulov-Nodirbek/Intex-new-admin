...
export default function Images({productPage,imagePage, atrPage}) {
        ...
        {img.map((item) => (
          <div key={item.id} className="relative inline-block w-40 h-28">
            <div className="absolute flex items-center space-x-1 top-1 right-1 cursor-pointer">
              ...
              <img id={item.id}  src={delterImg} alt="Delete img" width={32} height={32} />
            </div>
            <img className="h-full"  src={item.img} alt="Choose img" width="100%" height="100%" />
          </div>
        ))}
      </div>
